	...
	if Engine.get_physics_frames() % frame_update_spacing == frame_update_offset:
		...
		if smooth_occlusion:
			if time < smoothing_duration:
				...
				output_value = lerpf(volume_linear, frequency_value * (1.0 - occlusion_amount * occlusion_multiplier), time / smoothing_duration);
		...
		volume_linear = clampf(output_value, 0.0, 1.0);
