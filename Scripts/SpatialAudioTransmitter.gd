class_name SpatialAudioTransmitter;
...
var frequency_volume: float = 0.0;
	...
	if Engine.get_physics_frames() % frame_update_spacing == frame_update_offset:
		...
		if smooth_occlusion:
			if time < smoothing_duration:
				...
				volume_value = lerpf(volume_linear, (frequency_volume if use_frequency else base_volume) * (1.0 - occlusion_amount * occlusion_multiplier), time / smoothing_duration);

		else:
			volume_value = (frequency_volume if use_frequency else base_volume) * (1.0 - occlusion_amount * occlusion_multiplier);

		if not target:
			volume_value = 0.0;

		volume_linear = clampf(volume_value, 0.0, 1.0);
...
func update_occlusion() -> void:
	if playing and target and ((use_frequency and global_position.distance_to(target.global_position) < range_partial) or (not use_frequency and max_distance == 0.0) or (not use_frequency and global_position.distance_to(target.global_position) < max_distance)):
		if not base_volume:
			base_volume = volume_linear;
		...
		if collision:
			...
			if "physics_material_override" in collider:
					if collider.physics_material_override is MaterialProperties:
						occlusion_amount = collider.physics_material_override["signal_occlusion" if use_frequency else "occlusion"];
					else:
...
func update_strength(new_strength: float) -> void:
	if Engine.get_physics_frames() % frame_update_spacing == frame_update_offset:
		if new_strength < 0.01:
			frequency_volume = 0.0;
		elif new_strength > 0.99:
			frequency_volume = base_volume;
		else:
			frequency_volume = remap(new_strength, 0.0, 1.0, 0.0, base_volume);
